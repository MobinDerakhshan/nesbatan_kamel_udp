...
def run():
    ...
    middle_host.cmd(
        f"mid/limi2 {get_interface(MIDDLE_HOST_NAME, 0)} {get_interface(MIDDLE_HOST_NAME, 1)} &")
    
    #right_host.cmd(f"./right/build/right > right_log.txt &")
    #left_host.cmd(f"./left/build/left > left_log.txt &")

    right_host.cmd(f"./socketAlaki/build/socketAlaki > server_log.txt &")
    left_host.cmd(f"./clientSocketAlaki/build/clientSocketAlaki > client_log.txt &")
    net.start() 
    CLI(net)
    net.stop()
